...
public class GuardiansOfTheRiftHelperInactivePortalOverlay extends Overlay {
  ...
  @Inject
  public GuardiansOfTheRiftHelperInactivePortalOverlay(
      Client client,
  ...
  public Dimension render(Graphics2D graphics) {
    ...
    if (config.inactivePortalOverlayLocation() != TimerOverlayLocation.GameOverlay
    ...
    if (plugin.isFirstPortal()) {
      int timeSincePortalMillis =
          despawn.isPresent()
              ? ((int) (ChronoUnit.MILLIS.between(despawn.get(), Instant.now())))
              : 0;
